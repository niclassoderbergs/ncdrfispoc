        ...
        <div style={{...pocStyles.section, backgroundColor: '#f8fafd'}}>
            ...
            {isHowToExpanded && (
                <div style={{display: 'grid', gap: '10px', fontSize: '0.9rem', color: '#172b4d', lineHeight: '1.55', marginTop: '14px'}}>
                    <div><strong>Bid Level:</strong> The level at which the SP can submit flexibility bids.</div>
                    <div><strong>Note:</strong> SPU is a single-resource variant that is not yet included in this POC. The only difference is that an SPG contains multiple CUs, while an SPU contains one CU. They work the same way in the interface.</div>
                    <div><strong>SPG Details (Click ID):</strong></div>
                    <div><strong>Summary:</strong> Aggregated capacity from underlying CUs, number of CUs, and number of submitted bids.</div>
                    <div><strong>Market Readiness:</strong> Shows the SPG's qualified products (source for the information shown on the CU detail page).</div>
                    <div><strong>Recent Market Bids:</strong> Shows submitted bids.</div>
                    <div><strong>Verification Results (Ex-post):</strong> Shows activated bids and their verification results.</div>
                    <div><strong>Included Resources (CUs):</strong> Shows which CUs are included in the SPG.</div>
                </div>
            )}
        </div>
